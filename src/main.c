...
WRAP0(enforce_boundary_condition)
WRAP0(print)
WRAP0(run_analysis)
WRAP2(memory_usage)
...
#define METHOD(m) {#m, L_m2sim_##m}
  lua_struct_method_t methods[] = {
    ...
    METHOD(enforce_boundary_condition),
    METHOD(print),
    METHOD(run_analysis),
...
static int L_m2_tubomak_vector_potential(lua_State *L)
{
  double **x = (double **) luaL_checkudata(L, 1, "dvec4");
  double **n = (double **) luaL_checkudata(L, 2, "dvec4");
  double A = m2_tubomak_vector_potential(*x, *n);
  lua_pushnumber(L, A);
  return 1;
}
