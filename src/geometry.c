...
double m2_volume_measure(double x0[4], double x1[4], int geometry)
{
  ...
  case M2_CARTESIAN: dV = (x1[1] - x0[1])*(x1[2] - x0[2])*(x1[3] - x0[3]);
    break;
  case M2_CYLINDRICAL: dV = 0.0;
    MSG(FATAL, "cylindrical not yet implemented");
    break;
  case M2_SPHERICAL: dV = -1.0/3.0 *
      (x1[1]*x1[1]*x1[1] - x0[1]*x0[1]*x0[1]) *
      (cos(x1[2]) - cos(x0[2])) *
      (x1[3] - x0[3]);
    break;
  default:
...
double m2_area_measure(double x0[4], double x1[4], int geometry, int axis)
{
  ...
  case M2_SPHERICAL:
    switch (axis) {
    case 1: return -x0[1]*x0[1] * (cos(x1[2]) - cos(x0[2])) * (x1[3] - x1[3]);
