...
function hdf5.DataSet:value()
   ...
   elseif tcls == 'float' or tcls == 'int' then
...
function hdf5.DataSet:__index__(slice)
   ...
   elseif type(slice) == 'table' then
      local htype = self:get_type()
      if htype:type_class() ~= 'float' and htype:type_class() ~= 'int' then
	 error("DataSet:can only index data sets with class 'float' or 'int'")
      end
      --------------------------------------------------------------------------
...
function hdf5.DataSet:__init__(parent, name, mode, opts)
   ...
   if mode == "w" then
      ...
      local dtype = hdf5.DataType(opts.dtype or 'double')
