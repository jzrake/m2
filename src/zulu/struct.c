...
int _struct_api_methods(lua_State *L)
{
  int n = 0;
  if (lua_type(L, 1) == LUA_TSTRING) {
    luaL_getmetatable(L, lua_tostring(L, 1));
...
int _struct_api_type(lua_State *L)
{
  if (lua_isstring(L, 1) && lua_isstring(L, 2)) {
    luaL_getmetatable(L, lua_tostring(L, 1)); /* TOP: mt */
    lua_getfield(L, -1, "__members__"); /* TOP: mt.__members__ */
    lua_getfield(L, -1, lua_tostring(L, 2)); /* TOP: mt.__members__[...] */
    ...
    if (M == NULL) {
      ...
      case LSTRUCT_STRUCT: lua_pushstring(L, "struct"); break;
      }
      return 1;
    }    
  }
...
/*
 * If 'parent' is a non-negative number, the Lua value at that valid index will
 * be kept alive at least as long as the new reference.
 */
{
  if (parent != LSTRUCT_ORPHAN) parent = lua_absindex(L, parent);
  ...
  *obj = val;
  ...
  lua_getfield(L, -1, "__instances__"); /* TOP: mt.__instances__ */
  lua_newtable(L); /* TOP: mt.__instances__[obj] */
  if (parent != LSTRUCT_ORPHAN) {
    lua_pushvalue(L, lua_absindex(L, parent));
  }
  else {
    lua_pushboolean(L, 1);
  }
...
int _struct_constructor(lua_State *L)
{
  ...
  *obj = calloc(T->alloc_size, 1);
  ...
  lua_rawsetp(L, -2, obj); /* TOP: mt.__instances__ */
  lua_pop(L, 2); /* TOP: obj */
  lua_insert(L, 1); /* STACK: obj, arg1, arg2, ... */
  if (T->__init) {
    lua_pushvalue(L, 1);
    lua_insert(L, 1);
    ...
    lua_pop(L, lua_gettop(L)-1); /* kill off any args sitting on the stack */
  }
  return 1;
}
...
int _struct_mt_index(lua_State *L)
{
  ...
  if (memb) {
    ...
    case LSTRUCT_CHAR:
      lua_pushnumber(L, *((char *)(*obj + memb->offset)));
      break;
    case LSTRUCT_INT:
...
int _struct_mt_newindex(lua_State *L)
{
  ...
  if (key == NULL) {
    luaL_error(L, "illegal type '%s' for struct key",
