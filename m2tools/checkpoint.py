...
class MaraCheckpoint(object):
    """
    Class representing a Mara HDF5 checkpoint file
    """
    def __init__(self, filename, writable=False):
        import h5py
        self._file = None # so that instance has attribute even if open fails
        ...
        self._derived_fields['d'] = SingleField('rho')
        self._derived_fields['p'] = SingleField('pre')
        self._derived_fields['v1'] = SingleField('vx')
        self._derived_fields['v2'] = SingleField('vy')
        self._derived_fields['v3'] = SingleField('vz')
        self._derived_fields['B1'] = SingleField('Bx')
        self._derived_fields['B2'] = SingleField('By')
        self._derived_fields['B3'] = SingleField('Bz')
    ...
    def __repr__(self):
        return "<Mara checkpoint {0}>".format(self.filename)
    ...
    def domain_resolution(self):
        import numpy as np
        S = list(self._file['prim']['rho'].shape)
        S.insert(0, 0)
        while len(S) < 4:
            S.append(1)
        return np.array(S)
    ...
    @property
    def coordinate_scalings(self):
        return (None, 'linear', 'linear', 'linear')
    ...
    def cell_primitive(self):
        try:
            m2key = { 'rho': 'd',
                      ...
                      'By': 'B2',
                      'Bz': 'B3' }
            return dict([m2key[k],v] for k,v in self._file['prim'].iteritems())
        except KeyError: # it's a light checkpoint
            return { }
...
def Checkpoint(filename, writable=False):
    import h5py
    h5f = h5py.File(filename, 'r')
    if 'prim' in h5f:
        return MaraCheckpoint(filename, writable=writable)
    else:
        return M2Checkpoint(filename, writable=writable)
